...
class LSIAModel():
  ...
  def load(self, content, version):
    filename = f'./models/{content}-{version}.pkl'
    with open(filename, 'rb') as f:
      if content == 'lsia':
        [self.model] = pickle.load(f)
      elif content == 'model_ranker':
        [self.model_ranker] = pickle.load(f)
  
  def save(self, content, version):
    filename = f'./models/{content}-{version}.pkl'
    with open(filename, 'wb') as f:
      if content == 'lsia':
        pickle.dump([self.model], f)
      elif content == 'model_ranker':
        pickle.dump([self.model_ranker], f)
...
def process_search_result():
  revamp_docs = []
  queries, docs = read_result_from_search()
  for docs_result in docs:
    temp = []
    for doc in docs_result:
      with open(doc, 'r') as f:
        lines = f.readlines()
        single_line = ' '.join([line.strip() for line in lines])
        temp.append((doc, single_line))
    revamp_docs.append(temp)
    temp = []
  return queries, revamp_docs

def dump_result_into_file():
  queries, revamp_docs = process_search_result()
  for item in [('queries', queries), ('revamp_docs', revamp_docs)]:
    filename = f'./models/queries-docs/{item[0]}.pkl'
    with open(filename, 'wb') as f:
      pickle.dump(item[1], f)

def main():
  dump_result_into_file()
  for item in ['queries', 'revamp_docs']:
    filename = f'./models/queries-docs/{item}.pkl'
    with open(filename, 'rb') as f:
      if item == 'queries':
        queries = pickle.load(f)
      elif item == 'revamp_docs':
        docs = pickle.load(f)
  
  model = LSIAModel()
  model.load('lsia', 'v1')
  model.load('model_ranker', 'v1')
  for idx in range(len(queries)):
    model.get_scores(queries[idx], docs[idx])
